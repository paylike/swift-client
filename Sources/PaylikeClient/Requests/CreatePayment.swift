...
extension PaylikeClient {
            ...
            httpClient.sendRequest(
                ...
                do {
                    ...
                    let fallthroughHandler: (Result<PaylikeClientResponse, Error>) -> Void = { result in
                        do {
                            try handler(.success(result.get()))
                        } catch {
    ...
    private func _createPayment(
        ...
        var createPaymentResponse = try checkCreatePaymentResponse(response)
           ...
           !(action.isEmpty || fields.isEmpty)
        {
            ...
            let stringData = try checkFormResponse(formResponse)
            if !requestData.hints.isEmpty {
                var requestHints = requestData.hints
                if let responseHints = createPaymentResponse.hints,
                   !responseHints.isEmpty
     ...
     */
    private func preparingData(_ path: String?, _ requestData: CreatePaymentRequest) throws -> (URL, RequestOptions) {
        ...
        let requestOptions = try initRequestOptions(withData: JSONEncoder().encode(requestData))
     ...
     */
    private func checkCreatePaymentResponse(_ response: PaylikeResponse) throws -> CreatePaymentResponse {
     ...
     */
    private func checkFormResponse(_ response: PaylikeResponse) throws -> String {
        guard let statusCode = (response.urlResponse as? HTTPURLResponse)?.statusCode else {
            throw ClientError.InvalidURLResponse
        }
        guard let data = response.data else {
